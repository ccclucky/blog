...
3. 启动elasticseach镜像
   ...
   docker run --name elasticsearch -p 9200:9200 \
    ...
    -e ES_JAVA_OPTS="-Xms64m -Xmx128m" \
     -v /mydata/elasticsearch/config/elasticsearch.yml:/usr/shellare/elasticsearch/config/elasticsearch.yml \
    -v /mydata/elasticsearch/data:/usr/shellare/elasticsearch/data \
    -v /mydata/elasticsearch/plugins:/usr/shellare/elasticsearch/plugins \
...
docker run --name maxwell --restart=always  -d  zendesk/maxwell bin/maxwell  --user='cclucky' --password='1234'  --host='192.168.100.37'  --producer=rabbitmq --rabbitmq_user='guest' --rabbitmq_pass='guest' --rabbitmq_host='IP地址' --rabbitmq_port='5672' --rabbitmq_exchange='maxwell_exchange'  --rabbitmq_exchange_type='fanout' --rabbitmq_exchange_durable='true' --filter='exclude: *.*, include: aurora.t_article.article_title = *, include: aurora.t_article.article_content = *, include: aurora.t_article.is_delete = *, include: aurora.t_article.status = *' //运行MaxWell
```
...
2. 在/usr/local/nginx下创建nginx.conf文件，格式如下
   ...
   http {
       ...
       server {
           listen       80;
           server_name  blog-blog-green.vercel.app;
   
           location / {      
               # 将root路径注释掉或者设置为您的后端应用程序所在的路径
               # root   /path/to/your/backend/app;
               index  index.html index.htm; 
               try_files $uri $uri/ /index.html;    
           }
               
           location ^~ /api/ {     
               proxy_pass http://114.132.162.245:8080/;
               proxy_set_header   Host             $host;
               proxy_set_header   X-Real-IP        $remote_addr;            
               proxy_set_header   X-Forwarded-For  $proxy_add_x_forwarded_for;
           }
       ...
       server {
           listen       80;
           server_name  blog-admin-nine.vercel.app;
...
3. 启动nginx
   ...
   ```shell
   docker run --name nginx --restart=always -p 80:80 -d -v /mydata/nginx/nginx.conf:/etc/nginx/nginx.conf -v /mydata/vue:/usr/local/vue nginx 
   ```
